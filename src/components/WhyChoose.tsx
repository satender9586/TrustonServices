                ...
                <CarouselContent className="gap-4">
                    {
                        data.map((items, index) => (
                            <CarouselItem key={index} className=' md:basis-1/2 lg:basis-1/3 border  flex flex-col justify-center items-center max-w-sm p-6  bg-white  border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700'>
                                    <div>{<items.icon  className = "size-13 text-[#81CAEB]" />}</div>
                                    <h1 className='font-serif mt-4 text-[22px] text-[#AF0D5A] tracking-[1px]'>{items.title}</h1>
                                    <h1 className='font-serif text-[16px] mt-1 text-[#20A5EA] tracking-[1.9px] text-center'>{items.desc}</h1>
                            </CarouselItem>
                        ))
                    }
                </CarouselContent>
